...
It's not a good idea to autosave your workspace on exit and automatically load it at startup.

+ Can mask code issues that are only discovered later at the worst time.
+ Big objects (10s to 1000s of MB will take a long time to save and load)

Solution: Disable this default behavior

1. Tools
2. Global Options
3. ![Set 'Save workspace to .RData on exit:' to 'Never' <br> Uncheck 'Restore .RData into workspace at startup'](figures/rdata.PNG)

+ Be brave! Pressing ctrl+shift+f10 (Restart R) should not be scary.
+ It's a sign that you have set up your workflow for RR and can be confident in code.
...
R Markdown allows you to intermingle descriptive text and code easily!
```{r}
msg <- "This is literate programming :)"
print(msg)
```
What was the value stored in `msg`? `msg` is "`r msg`". Don't forget that R code can be executed inline as well as in chunks!
...
When dealing with parallel (or multicore) code, the seed must get set for each worker for repeatability. 
